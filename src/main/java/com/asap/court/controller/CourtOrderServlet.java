...
public class CourtOrderServlet extends HttpServlet {
	...
	private void placeOrder(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		CourtOrderVO courtOrderVO = new CourtOrderVO(memberVO, courtVO, courtOrdStat, courtOrdDate, courtOrdTime,
				courtOrdTimeEnd, totalPrice);

		// 判斷是否有已成立的訂單涵蓋此時間（同時間付款完成的其他訂單）
		if (courtOrderService_interface.checkOrderDateAndTime(courtNo, courtOrdDate, courtOrdTime,
				courtOrdTimeEnd) == false) {
			System.out.println("有到這裡2");
			req.getRequestDispatcher("court_orderFail.jsp?error=time").forward(req, res);
		} else {
			// 判斷是否有重複的訂單，避免系統錯誤
			if ((courtOrderService_interface.checkOrder(courtNo, courtOrdDate, courtOrdTime,
					courtOrdTimeEnd)) == false) {
	...
	private void payOrder(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		obj.setReturnURL("http://asportsap.ddns.net/ASAP/court/ecPayReturn.do"); // 使用時要記得換成外網
