  ...
  <body>
          ...
          <div style="margin: 20px 10px ;border-radius: 10px;padding: 20px; box-shadow: rgba(0, 0, 0, 0.02) 0px 1px 3px 0px, rgba(27, 31, 35, 0.15) 0px 0px 0px 1px;;">
            <table id="table_id" class="display">
              ...
              <tbody>
                ...
                <tr>
                  <td>${report.fRptNo}</td>
                  <td><fmt:formatDate value="${report.fRptTime}" pattern="yyyy-MM-dd HH:mm" /></td>
                  <td>${report.postNo}</td>
                  <td>${report.cmtNo}</td>
                  ...
                  <td>${report.backNo}</td>
                  <td>${report.fRptReplyTime}</td>
                  <td>
                  <c:choose>
       				   <c:when test="${report.postNo != null}">
           				 <button type="button" class="btn btn-success btn-sm getpost" id="lookup" style="border-radius:5px ; margin-bottom:10px" data-postno="${report.postNo}">查看</button>
       				     <button type="button" class="btn btn-primary btn-sm post-manage" style="border-radius:5px; margin-bottom:10px" data-toggle="modal" data-target="#postModal${report.fRptNo}" data-postno="${report.postNo}">處理</button>
       				   
       				   <!-- Post Modal -->
       				   <div class="modal fade" id="postModal${report.fRptNo}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
					        ...
					        <div class="modal-header">
					          ...
					          <div class="modal-body">
					            <div style="font-size: 16px;">
					              <label>是否移除貼文：</label>
					              <input type="radio" id="yes" name="remove" value="2" required>
					              <label for="yes">是</label>
					              <input type="radio" id="no" name="remove" value="1" >
					              <label for="no">否</label>
					            </div>
					            <div style="color: black;">
					              回覆內容：
					            </div>
					            <textarea class="postcustretext" type="text" style="width: 100%; height:150px" placeholder="請輸入回覆訊息"></textarea>
					            <div style="text-align:right">
					              <button type="button" class="btn btn-secondary btn-sm" style="border-radius: 5px;"
					                data-dismiss="modal">取消</button>
					              <button type="button" class="btn btn-primary btn-sm reply-post" style="border-radius: 5px;" data-frptno="${report.fRptNo}" data-postno="${report.postNo}">送出</button>
					            </div>
					          </div>
					      </div>
					    </div>
        			   ...
        			   <c:when test="${report.cmtNo != null}">
            		     <button type="button" class="btn btn-success btn-sm getcomment" id="lookup" style="border-radius:5px ; margin-bottom:10px" data-cmtno="${report.cmtNo}">查看</button>
        			   	 <button type="button" class="btn btn-primary btn-sm cmt-manage" style="border-radius:5px; margin-bottom:10px" data-toggle="modal" data-target="#cmtModal${report.fRptNo}" data-cmtno="${report.cmtNo}">處理</button>
        			   	<!-- Comment Modal -->
						<div class="modal fade" id="cmtModal${report.fRptNo}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
						  <div class="modal-dialog">
						    <div class="modal-content">
						      <div class="modal-header">
						        ...
						        <div class="modal-body">
						          <div style="font-size: 16px;">
						            <label>是否移除留言：</label>
						            <input type="radio" id="yes" name="report-type" required>
						            <label for="yes">是</label>
						            <input type="radio" id="no" name="report-type" value="2">
						            <label for="no">否</label>
						          </div>
						          <div style="color: black;">
						            回覆內容：
						          </div>
						          <textarea name="custretext" type="text" style="width: 100%; height:150px" placeholder="請輸入回覆訊息"></textarea>
						          <div style="text-align:right">
						            <button type="button" class="btn btn-secondary btn-sm" style="border-radius: 5px" data-dismiss="modal">取消</button>
						            <button type="button" class="btn btn-primary btn-sm reply-comment" style="border-radius: 5px" data-frptno="${report.fRptNo}" data-postno="${report.postNo}">送出</button>
						          </div>
						        </div>
						    </div>
						  </div>
    ...
    <script>
      ...
      $("button.getcomment").on("click", function(){
    	   ...
    	   $("button.reply-post").on("click", function(){
    		  var postNo = $(this).attr('data-postno');
    		  var frptNo = $(this).attr('data-frptno');
    		  var postStatus=$('input[name="remove"]:checked').val();
    		  var replyText = $(this).closest(".modal-content").find("textarea.postcustretext").val();
    	  ...
    	  $(".fs_alert_bg").on("click", function(e) {
			    if (e.target === this) {
			        $("#fs_alert").hide();
			    }
			});
