...
<body>
														...
														<div class="col-md-6">
															<ul class="cat-sublist">
																...
																<li><div class="typetennis" data-category="3"
																		data-column="itemTypeNo" style="cursor: pointer">
																		男下身<span class="products-count">(1)</span>
																	</div></li>
    ...
    $("#itemsearch").on("click", function(event) {
      ...
      $(document).ready(function() {
    	    // 使用事件委託的方式綁定點擊事件
    	    // 這裡使用 'body' 作為事件委託的父元素
    	    $('body').on('click', 'div[data-category]', function() {
    	    	var typecategory = $(this).data('typecategory');
    	        var category = $(this).data('category');
    	        var column = $(this).data('column');
    	        handleButtonClick(typecategory,category, column);
    	    });

    	    // 通用的函數來處理按鈕點擊
    	    function handleButtonClick(typecategory,category, column) {
    	        $.ajax({
    	            url: "ItemInfoServlet",
    	            type: "GET",
    	            data: { typecategory: typecategory, category: category, column: column, action: "category" },
    	            dataType: "json",
    	            success: function(data) {
    	                $("div#roworder").empty();
    	                var itemsHtml = "";
