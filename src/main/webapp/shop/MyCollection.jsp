							...
							<c:forEach var="mycollection" items="${list}">
								<tr>
									...
									<td><c:choose>
											<c:when test="${mycollection.itemInfoVO.itemAddStat == 0}">
												<a
													href="${pageContext.request.contextPath}/shop/AsapShopProduct.jsp?itemNo=${mycollection.itemInfoVO.itemNo}"><button
											...
											<c:when test="${mycollection.itemInfoVO.itemAddStat != 0}">
												<button type="button" disabled
													...
													name="action" value="seemore">查看商品</button>
											</c:when>
										</c:choose></td>
									<td><button type="button"
											class="btn btn-danger btn-sm removesave"
											style="border-radius: 5px" name="action" value="removesave"
											data-collectno="${mycollection.itemCollectNo}">取消收藏</button></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
